...
from BuzzMusicBot import YouTube, app
from BuzzMusicBot.core.call import BuzzBot
from BuzzMusicBot.misc import db
from BuzzMusicBot.utils.database import get_loop
from BuzzMusicBot.utils.decorators import AdminRightsCheck
from BuzzMusicBot.utils.inline.play import (stream_markup,
                                          telegram_markup)
from BuzzMusicBot.utils.stream.autoclear import auto_clean
...
async def skip(cli, message: Message, _, chat_id):
    if not len(message.command) < 2:
                        ...
                        for x in range(state):
                            ...
                            if not check:
                                try:
                                    ...
                                    await BuzzBot.stop_stream(chat_id)
                                except:
                                    return
                                break
                    else:
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await BuzzBot.stop_stream(chat_id)
                except:
                    return
        except:
            try:
    ...
    if "live_" in queued:
        ...
        try:
            await BuzzBot.skip_stream(chat_id, link, video=status)
        except Exception:
        ...
        try:
            await BuzzBot.skip_stream(chat_id, file_path, video=status)
        except Exception:
            return await mystic.edit_text(_["call_9"])
        ...
        try:
            await BuzzBot.skip_stream(chat_id, videoid, video=status)
        except Exception:
            return await message.reply_text(_["call_9"])
        button = telegram_markup(_)
        await message.reply_photo(
        ...
        try:
            await BuzzBot.skip_stream(chat_id, queued, video=status)
        except Exception:
            return await message.reply_text(_["call_9"])
        if videoid == "telegram":
            button = telegram_markup(_)
